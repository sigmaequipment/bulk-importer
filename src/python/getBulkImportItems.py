...
class BackgroundTaskBulkImport(threading.Thread):
    def getItemsForImport(self,*args,**kwargs):
        while True:
            ...
            bulk_import_query = requests.get(f'{PostgREST_Table_String}?select=*&and=(inventory_sku.gt.{sku_of_last_import},and(source.not.eq."MANUAL CREATION", source.not.eq."SERIES GENERATOR"))&order=inventory_sku.asc&limit=100').json()
